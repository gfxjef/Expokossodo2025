...
class RegistroSystemTester:
    ...
    def setup_test_data(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                print(f"[ERROR] Error obteniendo eventos: {response.status_code}")
                return False
            ...
            if len(self.test_events) < 6:
                print(f"[ERROR] Se necesitan al menos 6 eventos para testing, encontrados: {len(self.test_events)}")
                return False
            
            print(f"[OK] Encontrados {len(self.test_events)} eventos para testing")
            ...
            if response.status_code not in [200, 201]:
                print(f"[ERROR] Error creando usuario existente base: {response.status_code}")
                print(f"Response: {response.text}")
                return False
            
            print("[OK] Usuario existente base creado exitosamente")
            return True
            
        except Exception as e:
            print(f"[ERROR] Error en setup_test_data: {e}")
            return False
    
    def cleanup_test_data(self):
        """Limpiar datos de prueba creados durante los tests"""
        print("[CLEANUP] Limpiando datos de prueba...")
        ...
        try:
            ...
            print(f"[OK] Eliminados registros de prueba: {test_emails}")
    ...
    def test_caso_1_usuario_nuevo(self) -> Dict[str, Any]:
        ...
        """
        print("\n[TEST] Test Caso 1: Usuario Completamente Nuevo")
    ...
    def test_caso_2_usuario_existente_sin_conflictos(self) -> Dict[str, Any]:
        ...
        """
        print("\n[TEST] Test Caso 2: Usuario Existente - Sin Conflictos")
    ...
    def test_caso_3_usuario_existente_conflictos_parciales(self) -> Dict[str, Any]:
        ...
        """
        print("\n[TEST] Test Caso 3: Usuario Existente - Conflictos Parciales")
    ...
    def test_caso_4_usuario_existente_todos_conflictos(self) -> Dict[str, Any]:
        ...
        """
        print("\n[TEST] Test Caso 4: Usuario Existente - Todos Conflictos")
    ...
    def run_full_test_suite(self) -> Dict[str, Any]:
        ...
        """
        print("[START] Iniciando Suite Completa de Testing del Sistema de Registro")
        print("=" * 70)
        ...
        try:
            ...
            for test_func in test_cases:
                try:
                    ...
                    if result['success']:
                        ...
                        print(f"[OK] {result['test_name']} - PASSED ({result['performance_ms']:.2f}ms)")
                    else:
                        suite_result['tests_failed'] += 1
                        print(f"[ERROR] {result['test_name']} - FAILED")
                    ...
                    print(f"[ERROR] Error ejecutando {test_func.__name__}: {e}")
            ...
            if self.test_mode:
                try:
                    ...
                    print(f"[WARN] Error durante cleanup: {e}")
    ...
    def generate_test_report(self, suite_result: Dict[str, Any]) -> str:
        ...
        # Resumen ejecutivo
        report.append("[STATS] RESUMEN EJECUTIVO")
        report.append("-" * 20)
        ...
        report.append(f"Estado general: {'[OK] EXITOSO' if suite_result['overall_success'] else '[ERROR] FALLIDO'}")
        report.append("")
        
        # Detalles por test
        report.append("[DETAILS] DETALLES POR TEST")
        report.append("-" * 20)
        
        for test_result in suite_result['test_results']:
            report.append(f"\n[TEST] {test_result['test_name']}")
            report.append(f"Estado: {'[OK] PASSED' if test_result['success'] else '[ERROR] FAILED'}")
            report.append(f"Performance: {test_result['performance_ms']:.2f}ms")
            
            # API Validation
            api_val = test_result['api_validation']
            report.append(f"Validación API: {'[OK]' if api_val.get('success') else '[ERROR]'}")
            ...
            report.append(f"Validación BD: {'[OK]' if db_val.get('success') else '[ERROR]'}")
        ...
        if integrity_issues:
            report.append("[ERROR] Se encontraron problemas de integridad:")
            for issue in integrity_issues:
                report.append(f"    * {issue}")
        else:
            report.append("[OK] Todos los tests mantuvieron integridad de datos")
        ...
        if performances:
            ...
            if slow_tests:
                report.append("[WARN] Tests lentos (>2s):")
                for test in slow_tests:
                    report.append(f"  [SLOW] {test['test_name']}: {test['performance_ms']:.2f}ms")
        
        # Errores del suite
        if suite_result['errors']:
            report.append("\n[ERROR] ERRORES DEL SUITE")
            report.append("-" * 20)
        ...
        if suite_result['overall_success']:
            report.append("[OK] El sistema de registro está funcionando correctamente")
            report.append("[OK] Todas las validaciones de integridad pasaron")
            report.append("[OK] Los casos de conflicto se manejan apropiadamente")
        else:
            report.append("[ERROR] Se requiere atención en las áreas fallidas")
            report.append("[FIX] Revisar logs detallados para troubleshooting")
            
        if suite_result['average_performance_ms'] > 1500:
            report.append("[WARN] Considerar optimización de performance")
...
def main():
    ...
    print("[MAIN] ExpoKossodo 2025 - Registro System Tester")
    ...
    try:
        ...
        print(f"\n[REPORT] Reporte guardado en: {report_filename}")
        ...
        print(f"\n[ERROR] Error crítico durante testing: {e}")
